...
namespace DelaunayVoronoi
{
    public class DelaunayTriangulator
    {
        ...
        public IEnumerable<DVPoint> GeneratePoints(int amount, double maxX, double maxY)
        {
            MaxX = maxX;
            MaxY = maxY;

            // TODO make more beautiful
            var point0 = new DVPoint(0, 0);
            var point1 = new DVPoint(0, MaxY);
            var point2 = new DVPoint(MaxX, MaxY);
            var point3 = new DVPoint(MaxX, 0);
            var points = new List<DVPoint>() { point0, point1, point2, point3 };
            ...
            for (int i = 0; i < amount - 4; i++)
            {
                ...
                points.Add(new DVPoint(pointX, pointY));
            }

            return points;
        }

        public IEnumerable<Triangle> BowyerWatson(IEnumerable<DVPoint> points)
        {
        ...
        private Triangle GenerateSupraTriangle()
        {
            ...
            // v maxY
            var margin = 500;
            var point1 = new DVPoint(0.5 * MaxX, -2 * MaxX - margin);
            var point2 = new DVPoint(-2 * MaxY - margin, 2 * MaxY + margin);
            var point3 = new DVPoint(2 * MaxX + MaxY + margin, 2 * MaxY + margin);
            return new Triangle(point1, point2, point3);
        }

        private ISet<Triangle> FindBadTriangles(DVPoint point, HashSet<Triangle> triangles)
        {
